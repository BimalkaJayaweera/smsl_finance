...
class JournalmainController extends Controller
{
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post()) ) { 
            ...
            $cr_tot = 0;
            $tot_journal_amt = 0;
            foreach ($_POST['Journaldetail'] as $Item) 
            {

                if($Item['line_total_D'])
                {
                    $line_total_D = $Item['line_total_D'];
                    $line_total_D = str_replace(',', '', $line_total_D);
                    $dr_tot += $line_total_D;
                }
                else $line_total_D = 0;

                if($Item['line_total_C'])
                {
                    $line_total_C = $Item['line_total_C'];
                    $line_total_C = str_replace(',', '', $line_total_C);
                    $cr_tot += $line_total_C;
                }
                else $line_total_C = 0;
                ...
                $tot_journal_amt +=($line_total_D + $line_total_C);
            }
            ...
            if ($valid) 
            {
                ...
                try 
                {
                    ...
                    {                        
                        foreach ($_POST['Journaldetail'] as $Item) 
                        {
                            $modelJournalItem = new Journaldetail();
                            $modelJournalItem->attributes = $Item;
        
                            if($Item['line_total_D'])
                            {
                                $modelJournalItem->dr_or_cr = 'D';
                                $line_total_D = $Item['line_total_D'];
                                $line_total_D = str_replace(',', '', $line_total_D);
                            }
                            else $line_total_D = 0;
                            ...
                            if($Item['line_total_C'])
                            {
                                $modelJournalItem->dr_or_cr = 'C';
                                $line_total_C = $Item['line_total_C'];
                                $line_total_C = str_replace(',', '', $line_total_C);
                            }
                            else $line_total_C = 0;
        
                            $modelJournalItem->line_total = $line_total_D + $line_total_C;
                            $modelJournalItem->journal_main_id = $model->journal_id;     
                            //$modelJournalItem->save();             

                            if (! ($flag = $modelJournalItem->save())) 
                            {
                                $transaction->rollBack();
                                $err = $modelJournalItem->getErrors();
                                print($err);
                                break;
                            }                                                 
                        }
                    ...
                    if ($flag) 
                    {
                       ...
                       /* foreach ($modelsJournalItems as $modelJournalItem) 
                        ...
                        {
                            $modelentryDetail = new AccEntryDetail();
                            $modelentryDetail->char_of_acc_id = $Item['chart_of_acc_id'];
                            $modelentryDetail->entry_id = $modelentryMain->entry_id;
                            ...
                            $modelentryDetail->entry_amount = $line_total_D + $line_total_C;    

                            //$modelentryDetail->save(false);                 
                            if (! ($flag = $modelentryDetail->save())) 
                            {
                                $transaction->rollBack();
                                $err = $modelJournalItem->getErrors();
                                print($err);
    ...
    public function actionDelete($id)
    {
        ...
        $acc_entry_models = AccEntryMain::find()->where(['entry_type' => 'JE-'.$id])->all();

        foreach($acc_entry_models as $m)
        {
            $m->delete();
        }
